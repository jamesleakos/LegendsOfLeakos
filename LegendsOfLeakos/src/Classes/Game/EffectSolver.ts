...
import { ZoneEnum, ZoneEnumMethods, ZoneType } from '../../Enums/Zone';
import TargetType from '../Target/TargetType';
import RuntimeZone from '../Zone/RuntimeZone';
import { GameZoneType } from '../Zone/GameZone';
...
class EffectSolver {
  ...
  createFightTargetInfoList(attackedCardInstanceId: number): TargetInfo[] {
    ...
    let attackedCardTargetInfo = new TargetInfo(
      attackedCardList,
      [],
      false,
      false,
  ...
  doEffect(
    ...
    for (let p of this.gameState.players) {
      for (let zone of p.zones) {
        let zoneDefinition =
          this.gameState.gameManager.gameProperties.gameZones.find(
            (x: GameZoneType) => x.zoneEnum === zone.zoneEnum
          );
